...
1. **Multi-source BFS** — Compute the minimum Manhattan distance from each cell to the nearest thief:
   - Initialize queue with all thief cells (distance = 0)
   - BFS outwards, incrementing distance by 1 for each step

2. **Binary Search + BFS** — Search for the maximum safeness factor:
   - Binary search on `k` (0 to 2n)
   - For each `k`, use BFS to check if a path exists where every cell has `dist ≥ k`
   - Start/end cells must satisfy `dist ≥ k`

### Complexity

- **Time**: O(n² log n) — BFS in O(n²), binary search with log(2n) iterations
- **Space**: O(n²) — distance matrix + visited array
